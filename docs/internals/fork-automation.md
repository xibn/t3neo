...
- Almost everything lives in `apps/web` (the desktop app wraps the web client) plus a few desktop
  files for the pet window. Server, contract, and adapter changes are limited to the short list
  in the Constraints section of `FEATURE.md`; mobile is never touched. Keeping the footprint
  this small is what makes automatic reapplication reliable; the spec forbids widening it.
