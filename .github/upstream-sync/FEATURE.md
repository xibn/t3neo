...
A **Neo** tab in Settings (`NeoSettingsPanel.tsx`, preferences in `neo/neoSettings.ts`, stored
per client in localStorage `t3code:neo-settings:v1`, decoded field by field so one unknown value
never resets the others). Rows, each registered in `settingsSearch.ts`:

1. **Updates** (feature 3) and the repository card above every tab (`NeoVersionCard`).
2. **Default context window** (`defaultContextWindow: "biggest" | "smallest"`, default smallest)
   and **Default fast mode** (`defaultFastMode: "fastest" | "slowest"`, default slowest):
   `neo/neoModelDefaults.ts` (`applyNeoModelOptionDefaults`, applied in
   `composerProviderState.tsx`) biases a new chat's model option descriptors to the preference;
   a selection saved on an existing thread still wins. Tested in `neoModelDefaults.test.ts`.
3. **Show usage badges** (`usageBadges`, default on), **Queue messages while a turn runs**
   (`queueMessages`, default on) and **Confirm before discarding a queued message**
   (`queueDiscardConfirm`, default on): feature 1 and the usage badges below.
4. **Keep the composer expanded** (`composerExpanded`, default off): the composer grows with the
   text up to eight lines and keeps its tallest height until the message is sent or cleared;
   `ChatComposer` sets `data-neo-composer-expanded` and `looks/neo.css` only compacts the empty
   composer when it is unset (`:not([data-neo-composer-expanded])`).
5. **Header actions button** (`headerActionsToggle`, default on) and **Collapse header actions**
   (`headerActionsCollapsed`, default off): a double-chevron at the right end of the chat
   header's actions folds **Add action**, **Open**, the branch controls and **Commit** away.
   While folded the shared page header (`WorkspacePageHeader`, `data-neo-page-header`) slims to a
   strip and the page below it becomes a card with rounded top corners (`looks/neo.css`,
   `data-neo-header-collapsed`); the chevron and panel toggles sit in a notch at the card's
   top-right corner. Settings, Usage and Pull Requests always render this way under the Neo look
   and host their own controls (Restore defaults, the usage toggles) in the notch
   (`data-workspace-page-actions` in `routes/settings.tsx`).
6. **Branch manager position** (`branchToolbarPosition: "composer" | "header"`, default composer)
   moves the workspace + branch controls from below the composer into the chat header next to
   **Open** (`ChatHeader`'s `extraActions` slot, docked by `ChatView`); **Show the branch manager
   move pill** (`branchToolbarMoveButton`, default on) adds the amber pill that moves it either
   way. When the header gets tight the workspace label folds to its icon.
7. **Nightly builds.** Fork rebuilds of upstream nightlies are versioned
   `X.Y.Z-nightly.neo.<date>.<run>`; `scripts/build-desktop-artifact.ts` and
   `apps/desktop/src/updates/updateChannels.ts` match `-nightly.(?:neo.)?<date>.<run>` so they
   land on the **Nightly** update channel and are named **T3 Neo (Nightly)**. Stable installs and
   the download link never switch to them. Tested in `updateChannels.test.ts`.

## Pets

A **Pets** tab (`PetSettingsPanel.tsx`, route `settings.pets.tsx`) with a picker over
`PET_DEFINITIONS` (`neo/pets/petRegistry.ts`): **No pet** (`none`, preview a spinning ASCII X from
`spinningXFrames`), **Hoppy (Loop)** (`rabbit`, a CSS rabbit that hops in a loop; transform-only
keyframes, paused when not playing), **Wukong (Reactive)** (`wukong`, ASCII monkey: sleeps,
watches while the user types, hammers while a thread runs, `WukongPet.tsx`), and **Lunar (No
Animation)** (`lunar`, the still moon and ring glow, `LunarPet.tsx`), followed by imported Codex
pets (feature 3, item 8). Also on the tab: a **Pet size** slider (`petSize`, 32 to 360 px,
default 160) and an **ASCII pet color** select (`asciiPetColor: "system" | "light" | "dark"`,
`data-neo-ascii-color` on `<html>`, colors `--neo-ascii-pet-light/dark` in `neo/neo.css`, a
split-circle preview for System).

On desktop the pet lives in its own always-on-top, non-focusable, transparent window
(`routes/pet.tsx` → `PetWidget.tsx`; `apps/desktop/src/ipc/methods/pet.ts` `openPetWindow`,
exposed as the optional `desktopBridge.pet` in `preload.ts` and typed `pet?:` in
`packages/contracts/src/ipc.ts`; `usePetWindowSync` opens, swaps and closes it with the setting).
It is only as big as the pet with its bubble and two pills, draggable, and clicking the pet brings
the app forward. `usePetActivitySync` (pet window) feeds the mood from thread shells and the queue;
`usePetTypingSync` (main window, `routes/__root.tsx`) relays composer keystrokes over a
BroadcastChannel so typing counts in every window. The bubble names one thread with Codex's
labels, the first pill counts running threads (a check once finished work waits), the second
switches the bubble between one run and all. The web app has no pet window. The sidebar footer's
paw-print button opens the tab and is filled with the accent while a pet is on.
