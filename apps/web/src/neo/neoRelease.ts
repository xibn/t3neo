...
/**
 * The browser cannot tell an Apple silicon Mac from an Intel one; on desktop
 * the updater state carries the host arch (`hostArch`), which callers pass in.
 */
export function currentNeoDownloadPlatform(desktopArch: string | null = null): NeoDownloadPlatform {
...
export function neoDownloadForCurrentPlatform(
  release: NeoLatestRelease,
  desktopArch: string | null = null,
): {
  readonly platform: NeoDownloadPlatform;
  readonly asset: NeoReleaseAsset | null;
} {
  const platform = currentNeoDownloadPlatform(desktopArch);
