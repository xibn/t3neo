...
/**
 * Tells the pet about composer keystrokes. The composer lives in the main
 * window while the pet has its own window on desktop, so this is mounted in
 * the app shell (not the pet route) and `notePetTyping` broadcasts to the
 * other windows.
 */
export function usePetTypingSync(enabled: boolean): void {
  useEffect(() => {
    if (!enabled) return;
 ...
 * from the shells and the viewed thread from the route. Typing arrives from
 * the main window through `usePetTypingSync`. Mount once per pet window.
 */
