...
const decodeStoredPet = Schema.decodeUnknownOption(ImportedPetSchema);
...
export function readStoredImportedPets(
  ...
  try {
    ...
    // One unreadable entry must not empty the list: the next write would
    // persist that empty list over the user's pets.
    return entries.flatMap((entry) => {
      const pet = decodeStoredPet(entry);
      return Option.isSome(pet) && isImportedPetId(pet.value.id)
        ? [{ ...pet.value, id: pet.value.id }]
        : [];
    });
  } catch {
    return [];
