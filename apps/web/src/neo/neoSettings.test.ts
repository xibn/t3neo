...
describe("neo settings", () => {
  ...
  it("keeps the other stored choices when one value is unknown", () => {
    // A newer build's value after a downgrade must not reset everything else:
    // the next update would persist the defaults over the user's choices.
    const stored = readStoredNeoSettings({
      getItem: () =>
        JSON.stringify({
          usageBadges: false,
          pet: "wukong",
          petSize: 96,
          branchToolbarPosition: "floating",
          agentControlsStyle: "glass",
        }),
    });
