...
describe("useMessageQueueStore", () => {
  ...
  it("moves a forced message to the head of its thread so Send now skips the line", () => {
    const store = useMessageQueueStore.getState();
    const otherThread = ThreadId.make("thread-2");
    store.enqueue(makeMessage("x", "2026-09-05T09:59:00.000Z", otherThread));
    store.enqueue(makeMessage("a", "2026-09-05T10:00:00.000Z"));
    store.enqueue(makeMessage("b", "2026-09-05T10:00:01.000Z"));
    store.enqueue(makeMessage("c", "2026-09-05T10:00:02.000Z"));

    store.markSendNow(MessageId.make("c"));

    const ids = useMessageQueueStore.getState().messages.map((m) => m.id);
    expect(ids).toEqual(["x", "c", "a", "b"]);
    const byThread = useMessageQueueStore.getState().byThread;
    expect(byThread[queuedMessageThreadKey({ environmentId, threadId })]?.[0]?.sendNow).toBe(true);
    expect(
      byThread[queuedMessageThreadKey({ environmentId, threadId: otherThread })]?.[0]?.id,
    ).toBe("x");
  });
