...
export const useMessageQueueStore = create<MessageQueueStoreState>()((set, get) => {
  ...
  return {
    ...
    markSendNow: (messageId) => {
      ...
      if (!target) return;
      // The drain only ever looks at a thread's head, so a forced message moves
      // to the front of its thread's queue instead of waiting behind the others.
      const threadKey = queuedMessageThreadKey(target);
      const headIndex = messages.findIndex((entry) => queuedMessageThreadKey(entry) === threadKey);
      const rest = messages.filter((entry) => entry.id !== messageId);
      const reordered = [...rest.slice(0, headIndex), forced(target), ...rest.slice(headIndex)];
      set(withMessages(reordered, withoutPaused(get().pausedThreads, threadKey)));
    },
...
/**
 * Adopts what another tab or window of this origin wrote, so a message one
 * tab sent or discarded leaves every tab's queue at once instead of coming
 * back from a stale copy.
 */
export function reloadMessageQueueFromStorage(): void {
  const persisted = readPersistedQueue();
  useMessageQueueStore.setState({
    messages: persisted.messages,
    byThread: groupByThread(persisted.messages),
    pausedThreads: persisted.pausedThreads,
  });
}

if (typeof window !== "undefined") {
  window.addEventListener("storage", (event) => {
    if (event.key === null || event.key === MESSAGE_QUEUE_STORAGE_KEY) {
      reloadMessageQueueFromStorage();
    }
  });
}
