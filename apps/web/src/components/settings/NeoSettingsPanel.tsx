...
function LatestReleaseRow() {
  ...
  return desktopUpdates ? <DesktopUpdateRow /> : <WebReleaseRow />;
}

/** Only this row asks GitHub for the latest release; the updater row has no use for it. */
function WebReleaseRow() {
  const state = useLatestNeoRelease();
  const hostArch = useDesktopUpdateState()?.hostArch ?? null;
  ...
  const { platform, asset } = neoDownloadForCurrentPlatform(state.release, hostArch);
